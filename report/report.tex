...
Table~\ref{tab:results} summarises all baseline results.
Against a classical blind adversary (Unsupervised Wiener-Hunt), the
authorized Wiener decoder retains a $+2$--$+3$\,dB advantage.
However, state-of-the-art blind networks (NAFNet, Restormer) applied
\emph{zero-shot} — with no knowledge of the PSF — close this gap to
$\approx 0$\,dB, matching the authorized receiver entirely.
This demonstrates that the unoptimized ring-shaped PSF provides no
security against modern deep image priors, and directly motivates the
adversarial PSF optimization described in Section~\ref{sec:optimization}.

\begin{table}[H]
  \centering
  \caption{PSNR\,(dB) / SSIM for all baselines on three test images.
           Authorized methods (green) know the PSF; adversary methods
           (red) do not.}
  \label{tab:results}
  \small
  \begin{tabular}{l l
                  >{\columncolor{rowauth}}c
                  >{\columncolor{rowauth}}c
                  >{\columncolor{rowauth}}c}
    \toprule
    Role & Method & camera & astronaut & chelsea \\
    \midrule
    \multirow{2}{*}{\textsc{auth}}
      & Wiener (non-blind)       & 20.5 / 0.68 & 19.4 / 0.65 & 23.6 / 0.76 \\
      & DPIR (non-blind, deep)   & 20.5 / 0.68 & 19.3 / 0.65 & 23.5 / 0.76 \\
    \midrule
  \end{tabular}
  \vspace{2pt}
  \begin{tabular}{l l
                  >{\columncolor{rowadv}}c
                  >{\columncolor{rowadv}}c
                  >{\columncolor{rowadv}}c}
    \toprule
    Role & Method & camera & astronaut & chelsea \\
    \midrule
    \multirow{3}{*}{\textsc{adv}}
      & Wiener-Hunt (classical)       & 18.1 / 0.34 & 17.0 / 0.29 & 20.7 / 0.41 \\
      & NAFNet (zero-shot)            & 20.5 / 0.69 & 18.9 / 0.64 & 23.8 / 0.77 \\
      & Restormer (zero-shot)         & 20.4 / 0.69 & 18.8 / 0.63 & 23.7 / 0.76 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{Weeks~1--3 (complete).}
        Classical and deep baselines fully evaluated.
        Smooth SV-PSF forward model, Wiener non-blind decoder, Unsupervised
        Wiener-Hunt, DPIR, NAFNet, and Restormer all benchmarked.
        Key finding: SOTA blind networks (NAFNet, Restormer) close the security
        gap to $\approx 0$\,dB, confirming adversarial PSF optimization is necessary.
  \item \textbf{Month~2.}
        Integrate the differentiable PSF generator (co-worker's model).
        Build the adversarial optimization loop
        (Eq.~\ref{eq:objective}).
        Set up the DIV2K~\cite{agustsson2017div2k} training pipeline.
        Add LPIPS~\cite{zhang2018lpips} perceptual loss.
  \item \textbf{Month~3.}
        Ablation study over adversary strength and PSF parameterization.
        Identify a fabrication-candidate PSF with measurable security gap
        against SOTA blind networks.
