...
standard test images yield a security gap of approximately $\mathbf{9}$--$\mathbf{10}$\,dB
in PSNR between the authorized and blind recovery channels.  Our long-term
...
initialised with a Gaussian kernel of bandwidth $\sigma = 71/6 \approx 11.8$
pixels and is applied to the \emph{entire} blurred image, implicitly assuming
a single spatially-uniform blur.

Because the true PSF is spatially varying and ring-shaped, the estimated
kernel converges to a wrong, smeared average, and the recovered image is
...
decoder consistently achieves $\approx\!9$--$10$\,dB higher PSNR than the
blind adversary.
...
\begin{table}[H]
  ...
  \begin{tabular}{l
    ...
    \midrule
    camera    & 16.3 & 0.582 & 14.1 & 0.478 & 4.8 & 0.021 & \textbf{+9.3} \\
    astronaut & 15.6 & 0.542 & 14.4 & 0.475 & 4.7 & 0.028 & \textbf{+9.6} \\
    chelsea   & 18.6 & 0.643 & 16.0 & 0.536 & 5.8 & 0.016 & \textbf{+10.2} \\
    \bottomrule
  \end{tabular}
\end{table}
...
three images.  The adversary's output is uniform noise in all cases.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{results_camera.pdf}
  \caption{Results on the \emph{camera} image.
           Non-blind: 14.1\,dB / 0.478 SSIM.
           Blind: 4.8\,dB / 0.021 SSIM.}
  \label{fig:camera}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Results on the \emph{astronaut} image.
           Non-blind: 14.4\,dB / 0.475 SSIM.
           Blind: 4.7\,dB / 0.028 SSIM.}
  \label{fig:astronaut}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Results on the \emph{chelsea} image.
           Non-blind: 16.0\,dB / 0.536 SSIM.
           Blind: 5.8\,dB / 0.016 SSIM.}
  \label{fig:chelsea}
\end{figure}
...
\node[nloss]  (loss)  at (0, -5.0)  {Total loss $\mathcal{L}$\\
                                      {\scriptsize $\mathcal{L} = \mathcal{L}_\text{nb}
                                       + \lambda\mathcal{L}_\text{b}$}};
\node[nupd]   (upd1)  at (0, -6.45) {Update $D_\text{nb},\,D_\text{b}$\\
...
\begin{scope}[on background layer]
  \node[phasebox, fit=(img)(b1)(dnb1)(db1)(loss)(upd1)] (p1box) {};
\end{scope}
...
optical encryption key, with a security gap of approximately $9$--$10$\,dB
between an authorized non-blind decoder and a state-of-the-art blind
adversary.  The gap is fundamentally rooted in the spatial variation and
